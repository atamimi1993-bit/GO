  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">Browse storage facilities near your move.</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white border rounded-2xl">
          ...
          <p className="text-gray-500 text-sm">No storage facilities found. Try a different search.</p>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-4">
          {filtered.map(f => (
            <div key={f.id} className="bg-white border rounded-2xl p-5 hover:shadow-md transition-shadow">
              ...
              <p className="text-sm text-gray-600 flex items-center gap-1 mb-1">
                <MapPin size={14} className="text-gray-400" /> {f.address}, {f.city}, {f.state} {f.zip}
              </p>
              {f.phone && <p className="text-sm text-gray-500 flex items-center gap-1 mb-1"><Phone size={14} /> {f.phone}</p>}
              {f.website && (
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t">
                {f.price_range && <span className="text-sm text-gray-500">{f.price_range}</span>}
                {f.rating > 0 && (
